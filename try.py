...
"""
            ...
            try:
                ...
                def build_thread():
                    try:
                        ...
                        Clock.schedule_once(lambda dt: setattr(output_text, 'text', log_output), 0)
                    except Exception as e:
                        error_msg = f"Error building image: {str(e)}"
                        Clock.schedule_once(lambda dt: setattr(output_text, 'text', error_msg), 0)
    ...
    def pull_image(self, instance):
        """Pull a Docker image using the Python Docker SDK"""
        if not self._check_docker_client():
            return
        ...
        def do_pull_image(btn):
            try:
                ...
                output_text.text = f"Pulling image {image_name}...\nThis may take a while."
                
                # Use a thread to avoid UI freezing
                def pull_thread():
                    try:
                        # Pull image with Python SDK
                        self.docker_client.images.pull(image_name)
                        
                        # Update UI on main thread
                        Clock.schedule_once(lambda dt: setattr(
                            output_text, 'text', f"Successfully pulled image {image_name}"), 0)
                    except Exception as e:
    ...
    def _check_docker_client(self):
        ...
        if not self.docker_available:
            try:
                ...
                error_popup = Popup(
                    title='Docker Error',
                    content=Label(text=f"Could not connect to Docker: {str(e)}\n\nPlease make sure Docker is installed and running."),
                    size_hint=(0.7, 0.3)
                )
                error_popup.open()
                return False
...
class DockerImagesScreen(Screen):
            ...
            for image in images:
                ...
                if image.tags:
                    for tag in image.tags:
                        ...
                        # Add the repository
                        self.image_list.add_widget(Label(
                            text=repo,
                            size_hint_y=None,
                            height=40
                        ))
                        ...
                        run_btn.bind(on_press=lambda x, img=tag: self.run_container(img))
                        ...
                        remove_btn.bind(on_press=lambda x, img=tag: self.remove_image(img))
                    ...
                    remove_btn.bind(on_press=lambda x, img_id=image_id: self.remove_image_by_id(img_id))
                    action_layout.add_widget(remove_btn)
    ...
    def run_container(self, image_name):
        ...
        port_input = TextInput(hint_text="host:container (e.g. 8080:80)", size_hint_x=0.7)
        ...
        # Status output
        status_label = Label(text=f"Running image: {image_name}", size_hint_y=None, height=30)
                    ...
                    try:
                        ...
                        Clock.schedule_once(
                            lambda dt: setattr(status_label, 'text', 
                                          f"Container started: {container.short_id}"), 0
                        )
        ...
        def do_pull_image(btn):
            ...
            def pull_thread():
                try:
                    ...
                    Clock.schedule_once(
                        lambda dt: setattr(status_label, 'text', 
                                      f"Successfully pulled {image_name}"), 0
                    )
                    Clock.schedule_once(lambda dt: setattr(pull_btn, 'disabled', False), 0)
                    Clock.schedule_once(lambda dt: self.update_image_list(), 1)
...
class DockerContainersScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Filter container
        filter_container = BoxLayout(orientation="horizontal", size_hint=(1, None), height=40, spacing=10)
    ...
    def toggle_show_all(self, instance):
        ...
        self.show_all_btn.text = f"Show All Containers: {'Yes' if self.show_all else 'No'}"
        self.update_container_list()
    ...
    def update_container_list(self):
        ...
        try:
            ...
            for container in containers:
                ...
                self.container_list.add_widget(Label(
                    text=container_id,
                    size_hint_y=None,
                    height=40
                ))
                ...
                if container.status == "running":
                    ...
                    stop_btn.bind(on_press=lambda x, c=container.id: self.stop_container(c))
                    ...
                    start_btn.bind(on_press=lambda x, c=container.id: self.start_container(c))
                    ...
                    remove_btn.bind(on_press=lambda x, c=container.id: self.remove_container(c))
                    action_layout.add_widget(remove_btn)
    ...
    def view_logs(self, container_id):
        ...
        try:
            ...
            refresh_btn.bind(on_press=lambda x: threading.Thread(target=get_logs_thread).start())
            ...
            # Create and open popup
            popup = Popup(title=f"Logs - {container.name}", content=layout, size_hint=(0.9, 0.9))
            popup.open()
    ...
    def run_container(self, instance):
        ...
        # Image selection
        image_layout = BoxLayout(orientation="vertical", size_hint_y=None, height=80, spacing=5)
        image_layout.add_widget(Label(text="Select or enter image:", size_hint_y=None, height=20))
        ...
        # Status message
        status_label = Label(
            text="Configure container parameters",
            size_hint_y=None,
            height=40
        )
        layout.add_widget(status_label)
            ...
            try:
                ...
                def run_thread():
                    try:
                        ...
                        Clock.schedule_once(
                            lambda dt: setattr(status_label, 'text',
                                          f"Container {container.name} started successfully"), 0
                        )
