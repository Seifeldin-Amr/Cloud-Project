...
class CloudManagementSystem:
    def __init__(self, root):
        ...
        title_label = ttk.Label(
            self.main_frame, text="Cloud Management System", font=("Arial", 18, "bold")
        )
    ...
    def create_vm_form(self):
        ...
        form_frame = ttk.LabelFrame(
            self.create_vm_tab, text="Virtual Machine Configuration", padding="10"
        )
        form_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # System resources section
        resources_frame = ttk.LabelFrame(
            form_frame, text="System Resources", padding="10"
        )
        ...
        disk_frame = ttk.LabelFrame(
            form_frame, text="Virtual Disk Configuration", padding="10"
        )
        ...
        browse_btn = ttk.Button(
            path_frame, text="Browse", command=self.browse_storage_path
        )
        ...
        create_button = ttk.Button(
            button_frame, text="Create VM", command=self.create_vm
        )
        ...
        self.progress = ttk.Progressbar(
            output_frame, variable=self.progress_var, maximum=100, mode="determinate"
        )
    ...
    def create_vm(self):
        ...
        if not os.path.exists(storage_path):
            try:
                ...
                messagebox.showerror(
                    "Error", f"Could not create storage directory: {e}"
                )
                return
        ...
        threading.Thread(
            ...
            args=(disk_size, disk_format, storage_path, disk_name),
        ).start()
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                with open(bat_path, "w") as f:
                    ...
                    f.write(
                        f"start \"\" C:\\msys64\\mingw64.exe {' '.join(qemu_cmd)}\n"
                    )
                ...
                messagebox.showerror(
                    "Error", f"Failed to create virtual disk: {stderr}"
                )
